<template>
    <div class="d-flex flex-column">
        ...
        <div v-if="bookings && bookings.length > 0" class="d-flex flex-wrap justify-content-start">
            ...
            <div v-for="booking in bookings" v-bind:key="booking._id" class="card mb-2 ml-2 text-white bg-dark" style="width: 18rem;">
                <div class="card-body">
                    ...
                    <h6 class="card-subtitle mb-2 text-muted">
                        Athlete: {{ booking.title }}
                    </h6>


                    <p class="card-text2">Location: {{ booking.location }}</p>
                    <div v-if="booking.author._id === $store.state.userId" class="d-flex justify-content-between">
                        <router-link type="button" tag="button" class="card-link btn btn-primary" :to="{ name: 'bookings-edit', params: { id: booking._id } }"
                            exact>Edit</router-link>
                        <a v-on:click.prevent="currentBookingId = booking._id" class="card-link btn btn-danger" href="#" v-b-modal.modal1>Delete</a>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            deleteBooking: async function() {
                ...
                const index = this.bookings.findIndex(booking => booking._id === this.currentBookingId);
